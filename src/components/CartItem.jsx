...
const CartItem = ({ item, onRemove }) => {
  const handleRemove = () => { onRemove(item.id); toast.error(`${item.name} removed from cart`, { theme: 'light' }); };
  ...
  return (
    <div className="flex items-center gap-4 p-4 bg-white border border-gray-200 rounded-xl hover:border-[#4F39F6]/30 transition-colors">
      <div className="w-10 h-10 rounded-xl bg-[#EDE9FE] flex items-center justify-center flex-shrink-0 p-2">
        {iconSrc ? <img src={iconSrc} alt={item.name} className="w-full h-full object-contain" /> : <span className="text-xl">🛠️</span>}
      </div>
      ...
      <button onClick={handleRemove} className="text-xs font-semibold text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors flex-shrink-0">Remove</button>
    </div>
  );
};
